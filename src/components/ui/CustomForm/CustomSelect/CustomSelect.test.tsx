import { render, screen } from '@testing-library/react';
import '@testing-library/jest-dom'; // For the toBeInTheDocument matcher
import CustomSelect from './CustomSelect';

describe('CustomSelect', () => {
  // Test 1: Renders the component without crashing
  it('renders without crashing', () => {
    render(<CustomSelect data-testid="customSelect" />);
    expect(screen.getByTestId('customSelect')).toBeInTheDocument();
  });

  // Test 2: Renders children correctly
  it('renders children passed to it', () => {
    const testChildText = 'Hello World';
    render(<CustomSelect>{testChildText}</CustomSelect>);
    expect(screen.getByText(testChildText)).toBeInTheDocument();
  });

  // Test 3: Applies custom className
  it('applies a custom className', () => {
    const customClass = 'test-class';
    render(<CustomSelect className={customClass} />);
    expect(screen.getByTestId('customSelect')).toHaveClass(customClass);
  });

  
});
